...
class ExplainableGP(object):
    ...
    def __init__(self, config_dict, file_dict, gp_ops_distribution_list, monitor_dict):
        ...
        self.hash_tree_meta = {}            # All tree informations ->
        """"
        {tree_identifier: 
        {tree_identifier: tree_ident, 
        'algo_raw': algo_raw, 
        'algo_sym': algo_sym, 
        'parsimony': parsimony, 
        'fitness_train': fitness_train}}
        """
        self.pareto_front = {}              # the best tree for each distance value -> {distance: tree_meta_tuple}

        # self.origin_dominators = {}  # all Trees which share the best fitness score -> {tree_ident: tree_meta_tuple}

        self.class_labels = 0               # the number of true class labels (data_y) todo warum 0?
        self.population_selection = []       # population (with all the candidates from the genepool)
        ...
        self.monitoring_dict = {}                   # first gen only todo: what if we restart?
        self.gene_pool = []
        return
    ...
    def main_generation_first_origin(self, population_backup_file='', paretofront_backup_file=''):
        ...
        self.data_save_population(self.population_selection, '1_first')        # first gen only
    ...
    def main_terminate(self):
        ...
        self.data_write_file_paretofront()
        target = open(self.filename['f'], 'w')
    ...
    def data_load_origin_tree(self, origin_tree_file_path):
        ...
        self.origin['fitness_train'] = self.eval_fitness_traindata(self.origin['algo_raw'], self.origin['algo_sym'])
        ...
        with open(population_backup_file, 'rb') as csv_file:
            ...
            for row in target:
                ...
                if n == 1:
                    ...
                    self.population_selection = [row]  # write header to population_genepool
                    ...
                    if self.tree.shape[0] == TRn_um_lines:  # (current tree rows + row 0
                        self.population_selection.append(self.tree)  # append complete Tree to population list

        self.printpl('i', 'We loaded the following population_genepool:', self.population_selection)
        return
    ...
    def data_pickle_save(self):
        ...
        run_data = {'gen_id': self.gen_id,
                    ...
                    'hash_trees_meta': self.hash_tree_meta,
    ...
    def data_pickle_recover(self, samples_file, operators_file, origin_tree_file_path, paretofront_file):
        ...
        self.printpl('i', 'Recovered gene_pool:', self.population_selection, 'with size', len(self.population_selection))
    ...
    def data_save_conclusion(self):
        ...
        self.origin['result'] = self.eval_tf(self.origin['algo_sym'], self.data_test, get_pred_labels=True)
        self.origin['fitness_test'] = self.origin['result']['fitness']

        fitness_best = self.origin['fitness_test']
        fittest_algo = self.origin['algo_sym']
        fittest_parsimony = self.origin['parsimony']

        for parsim_key in self.pareto_front:

            tmp_algo = self.pareto_front[parsim_key]['algo_sym']
            tmp_result = self.eval_tf(self.pareto_front[parsim_key]['algo_sym'], self.data_test, get_pred_labels=True)
            tmp_fitness_test = tmp_result['fitness']
            tmp_parsimony = self.pareto_front[parsim_key]['parsimony']
            ...
            # TODO LARGE TODO These informations should find back into the result
            # if self.kernel == 'c':
    ...
    def data_write_file_paretofront(self):
        """
        ...
        for parsim_key in sorted(list(self.pareto_front.keys())):
            fitness = self.pareto_front[parsim_key]['fitness_train']
            algo_sym_str = self.pareto_front[parsim_key]['algo_sym']
    ...
    def pop_genepool_create(self, population):
        ...
        for tree_id in range(1, len(population)):  # Every tree
            ...
            tree_meta = self.tree_meta_get(tree)

            self.gene_pool.append(int(tree_id))
            # print(tree_meta, self.parsimony_min_max[1])
            if tree_meta['parsimony'] < self.parsimony_min_max[1]:  # Tree -> gene_pool?
                if int(tree_id) != int(tree[0][1]):
                    self.printpl('e', 'Delete this, this should never happen i guess^^')
                self.tree_store_parsimony(tree, tree_meta['parsimony'])
                gene_pool_meta_dict[tree_id] = tree_meta

        self.monitor_performance_generation(gene_pool_meta_dict)

        return gene_pool_meta_dict

    def monitor_performance_generation(self, gene_pool_meta_dict):
        """
    ...
    def pop_pareto_update(self, gene_pool_meta_dict):
        ...
        for tree_id in gene_pool_meta_dict:
            ...
            if parsim in self.pareto_front:
                cmp_fitness = self.pareto_front[parsim]
                if (self.fitness_type == 'max' and fitness > cmp_fitness) or \
                        (self.fitness_type == 'min' and fitness < cmp_fitness):

                    self.printpl('v', 'Found new tree with parsimony', parsim, 'and fitness', fitness, '. Old one was:', cmp_fitness)
                    #
                    # # delete the old tree from the paretofront
                    # del_hash = self.pareto_front[tmp_parsim]
                    # del self.pareto_front[del_hash]

                    # update everything to the new tree
                    self.pareto_front[parsim] = gene_pool_meta_dict[tree_id]
    ...
    def tree_meta_get(self, tree):
        ...
        tree_ident = hash(algo_raw_str)
        if tree_ident in self.hash_tree_meta:
            self.printpl('iii', 'It is good that this happened')
            # return self.hash_tree_meta[tree_ident]
        ...
        if parsimony < self.parsimony_min_max[1]:
            ...
            fitness_train = self.eval_fitness_traindata(algo_raw_str, algo_sym)
        else:
    ...
    def pop_copy_genepool(self, pop, gene_pool):
    ...
    def pop_tree_store_fitness_parsimony_analysis(self, population, gene_pool_meta_dict):
        """
        ...
        for tree_id in gene_pool_meta_dict:
            ...
            self.tree_store_fitness(tree, fitness_train)
            self.tree_store_parsimony(tree, gene_pool_meta_dict[tree_id]['parsimony'])
            self.tree_store_meta_lastgen(tree)

            if (fitness_train > self.origin['fitness_train'] and self.fitness_type == 'max') \
                    or (fitness_train < self.origin['fitness_train'] and self.fitness_type == 'min'):
                self.printpl('v', 'Found a candidate that is fitter than the origin')
                dominator_count += 1

        self.printpl('i', dominator_count, ' Candidates were better than the origin.')
        # self.printpl('i', '\033[36m ', len(list(self.origin_dominators.keys())), 'trees\033[1m dominate the origin. Training Fitness:',
        #              sorted(list(self.origin_dominators.values()), key=lambda x: x[1]), '\033[0;0m\033[36m compared to', self.origin['fitness_train'], '.\033[0;0m')
        return
    ...
    def gp_selection_tournament(self, tourn_size):
        ...
        for n in range(tourn_size):
            ...
            fitness = float(self.population_selection[tree_id][12][1])  # extract the fitness from the array
            ...
            if self.fitness_type == 'max':  # if the fitness function is maximising
                ...
                tourn_winner = np.copy(self.population_selection[tourn_lead])  # copy full Tree so as to not inadvertantly modify the original tree
        ...
        for j in range(1, len(self.tree[3])):  # go through all nodes
            if int(self.tree[4][j]) == self.pop_node_depth - 1:  # this node is a parent
                for k in range(1, (int(self.tree[TRn_arity][j]) + 1)):  # increment through each degree of arity for each parent node
                    ...
                    self.gp_mutate_branch_terminal_gen(function_dtypes_dict[self.tree[TRn_label][j]])  # ... generate a Terminal node
    ...
    def evolve_subtree_build_insert(self, winner_tree, branch_nodes):
        ...
        winner_tree[TRn_label][branch_top] = self.tree[TRn_label][1]  # copy node_label from new tree
        winner_tree[TRn_arity][branch_top] = self.tree[TRn_arity][1]  # copy node_arity from new tree
        ...
        while node_count < len(self.tree[3]):  # increment through all nodes in the new Tree ('gp.tree'), starting with node 2
            ...
            for j in range(1, len(winner_tree[3])):  # increment through all nodes in tourn_winner ('tree')
                ...
                if winner_tree[5][j] == '':
                    ...
                    winner_tree[TRn_label][j] = self.tree[TRn_label][node_count]  # copy 'node_label' from branch to tree
                    winner_tree[TRn_arity][j] = self.tree[TRn_arity][node_count]  # copy 'node_arity' from branch to tree
    ...
    def gen_finalize(self):
        ...
        gene_pool_meta_dict = self.pop_genepool_create(self.population_new)
        self.pop_tree_store_fitness_parsimony_analysis(self.population_new, gene_pool_meta_dict)     # gene +fitness

        self.pop_pareto_update(gene_pool_meta_dict)

        self.population_selection = self.pop_copy_genepool(self.population_new, self.gene_pool)
    ...
    def eval_fitness_traindata(self, algo_raw_str, algo_sym):
        """
        returns the fitness of a tree (and tries not to compute it with tensorflow)
        - kick out dummy
        ...
        algo_sym = str(algo_sym)
        tree_ident = hash(algo_raw_str)
        if tree_ident in self.hash_tree_meta:
            return self.hash_tree_meta[tree_ident]

        # 2. calculate the fitness and store it in the hash table
        fitness = self.eval_tf(algo_sym, self.data_train)['fitness']
        self.hash_tree_meta[tree_ident] = fitness
        return fitness
    ...
    def eval_tf_expr_graph(self, node, tensors):
        ...
        if isinstance(node, ast.Name):  # <tensor_name>
            ...
            if node.func.id in functions_multiparam_dict:  # Min, Max are now Mini and Maxi. These only accept two inputs.
                # self.printpl('e', [self.tf_expr_graph(arg, tensors) for arg in node.args])
                return operator_dict[node.func.id]([self.eval_tf_expr_graph(arg, tensors) for arg in node.args])  # the star '*' makes the difference
            ...
            try:
                ...
                self.printpl('w', 'node.func.id caused an exception:', node.func.id, 'and expression:', self.agen_debug_warning)
    ...
    def plagih_pause(self):
        ...
        menu_dict = {'input_a': '',
                     ...
                     'pop_last_len': len(self.population_selection),
        ...
        if input_a == 'esc':
            ...
            self.display_tree(self.population_selection[input_b])
            ...
            self.printpl('o', '')
            for tree_id in range(1, len(self.population_selection)):
                algo_sym = self.tree_expr_sympify(tree=self.population_selection[tree_id])
                self.printpl('i', '\t\033[36m Tree', self.population_selection[tree_id][0][1], 'yields (sym):\033[1m', algo_sym, '\033[0;0m')
    ...
    #     obsolete?
    #     """
    #     algo_sym = self.tree_expr_sympify(tree=self.population_new[tree_id])
    #
