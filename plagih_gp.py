...
def main():  # argv sys.argv[1:]
    ...
    gp = ExplainableGP(conf, rootdir, kernel, path_origin)
...
def _test_random_pop():
    ...
    run_name = 'MTC200_RMSE_scratch'
    rootdir = Path.cwd() / f'{run_name}'

    path_data_csv = Path(__file__).parent.absolute() / f'benchmarks/mc/gp_files/samples200.csv'
    path_origin = [None,
                   Path(__file__).parent.absolute() / f'benchmarks/mc/gp_files/tree_simple.txt',
                   Path(__file__).parent.absolute() / f'benchmarks/mc/gp_files/tree_simple_fix.txt'][0]
    ...
    complexity_measure = len  # tree_edit_distance / 'tree_node_count'  # sfeh idea
    ...
    kernel = RegressionKernel(use_RMSE_vs_MAE_dummy_todo, data_train, data_control, action_name)
    ...
    gp = ExplainableGP(conf, rootdir, kernel, path_origin, tb)
    time_start2 = time.perf_counter()
    gp.printpl('gg', f'Init. Time: {time.perf_counter() - time_start2:4.2f}s')
