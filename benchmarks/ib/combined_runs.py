...
def mp_evall(arow):
    ...
    try:
        ...
        print(
            f"Combined parsimony {parsim_sum:3.0f} regression-error: {regress_sum:.4f}. "
            f"\t({parsims})"
            f"\tcomplete: {ibx:0.1f} "
...
def eval_and_lut(eval_list, parsim_MAX, parsim_1MAX, lut_file, mp_cpu_MAX):
    ...
    mp_result_dict = {
        a: [b, c, d, e] for a, b, c, d, e in mp_result
    }  # [lut_hash, experiment, experiment_safe, experiment_r50, experiment_safe_r50]
...
def plot_best_prediction(rootdir_eval, parsims, combined_all_p, lut_file, parsim_MAX, parsim_1MAX, mp_cpu_MAX):
    ...
    bestregr_data = [
        [
            " ".join(f"{xx:0.0f}" for xx in x["parsims"]),
            x["experiment"],
            x["experiment_safe"],
            x["experiment_r50"],
            x["experiment_safe_r50"],
        ]
                ...
                if runfolders.get_name[:2] == "IB":
                    ...
                    try:
                        combined_lists(
                            ...
                            parsim_max_sum,
                            parsim_max_single,
                            local_yamls=args.locallut,
                            cpu_cores=args.mp_cores,
                        )
                    except Exception as ex:
