...
def thesis_plot_mc_comparisson():
    ...
    with plt.rc_context(rc=pyplot_rc_tex):
        ...
        savepath = path_make_dir(Path.cwd() / "benchmarks/MC/agents/img/mc_reward_compared.pdf")
...
def thesis_plot_ib_comparisson():
    ...
    with plt.rc_context(rc=pyplot_rc_tex):
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(names, rotation=60, fontsize=11)
        savepath = path_make_dir(Path.cwd() / "MA_lyx/img/IB/ib_reward_compared.pdf")
        fig.savefig(savepath)
        plt.close("all")
...
if __name__ == "__main__":
    print("Making all the plots for the thesis (so they are python-style)")
