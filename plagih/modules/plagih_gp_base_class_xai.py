...
class Base_GP(object):
    ...
    def __init__(self, kernel, tree_type, tree_depth_base, tree_depth_max, tree_depth_min, tree_pop_max, gen_max,
        ...
        self.parsimony_front_fitness = {}
        self.pareto_front_hash = {}
        self.best_trees = {}
    ...
    def write_paretofront(self):
        ...
        for key in self.parsimony_front_fitness:
            value = self.parsimony_front_fitness[key]
            tmp_hash = self.pareto_front_hash[key]
    ...
    def pop_pareto_front(self):
        ...
        for candidate in self.gene_pool:
            ...
            if tmp_parsim in self.parsimony_front_fitness:
                cmp_fitness = self.parsimony_front_fitness[tmp_parsim]
                if (self.fitness_type == 'max' and tmp_fitness > cmp_fitness) or \
                    ...
                    # delete old tree
                    old_tree_hash = self.pareto_front_hash[tmp_parsim]
                    del self.best_trees[old_tree_hash]

                    # update everything to the new tree
                    self.best_trees.update({expr_hash: algo_sym_str})
                    self.parsimony_front_fitness[tmp_parsim] = tmp_fitness
                    self.pareto_front_hash[tmp_parsim] = expr_hash
                ...
                self.best_trees.update({expr_hash: algo_sym_str})
                self.parsimony_front_fitness.update({tmp_parsim: tmp_fitness})
                self.pareto_front_hash.update({tmp_parsim: expr_hash})
