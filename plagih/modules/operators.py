...
def oparray_from_list(operator_tuples):
    """
    Load all operators ready-to-use from a file
    """

    xtype_index = lambda x: ['f2f', 'f2b', 'b2b', 'b2f', 'b2f2f'].index(x)
    ...
    for xtype, xrow in choose_oparray2.items():
        for arity in xrow.keys():
            ...
            # from a list of tuples [[op, prob], [+, 1], ...] to two lists [operators] [probability]
