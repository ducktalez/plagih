...
def mtc_approach_start(approach):
    ...
    plt.ylim(-200, -50)
    plt.savefig('MTC-'+str(approach)+'-'+str(episodes)+'-'+str(rewardSample_interval)+'.jpg')
    # plt.show()
    # plt.close()
    plt.show()

    plt.plot(rewardSample_interval * (np.arange(len(episode_rewards1)) + 1), rewards1_avg, label=str(approach))
    plt.legend()
    # plt.yscale('linear')
    plt.xlabel('Episodes')
