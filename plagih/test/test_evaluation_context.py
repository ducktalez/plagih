...
from plagih.trees import Add, Mul, Number, Symbol, Sin, Div
from plagih.evaluation_context import (
    EvaluationContext, EvaluationResult, EvalMode,
    create_context, evaluate_tree, add_unified_evaluation_to_node
)
...
def multi_var_tree():
    ...
    return Add(
        ...
        Mul(Symbol(sympy.Symbol('b')), Number(2.0))
    )
...
class TestEvaluationResult:
    ...
    def test_get_method(self):
        ...
        result = EvaluationResult(
            sympy=sympy.Symbol('a'),
            numpy_eager=np.array([1, 2, 3])
        )
