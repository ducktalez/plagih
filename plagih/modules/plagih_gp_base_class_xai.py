...
class ExplainableGP(object):
    ...
    def __init__(self, config_dict, file_dict, evolve_ratio_dict, monitor_dict):
        ...
        self.parsimony_best_hash = {}              # the best tree for each distance value -> {distance: tree_meta_tuple}
        self.pareto_front = {}
    ...
    def main_generation_first_origin(self, population_backup_file='', paretofront_backup_file=''):
        ...
        self.data_save_population(self.population_base, '1_first')        # first gen only
    ...
    def data_save_conclusion(self):
        ...
        for parsim_key, tree_hash in self.pareto_front.items():
    ...
    def data_write_file_paretofront(self):
        ...
        for parsim_key, tree_hash in sorted(list(self.pareto_front.items())):
            tree_meta = self.hash_tree_meta[tree_hash]
    ...
    def pop_pareto_update(self):
        ...
        best_fitness = self.origin['fitness_train']
        for parsim, tree_meta in sorted(list(self.parsimony_best_hash.items())):
            fitness_train = self.hash_tree_meta[tree_meta]['fitness_train']
            if self.fitness_compare(fitness_train, best_fitness, mode='better_or_equal'):
                self.printpl('v', 'Pareto updated at parsimony', parsim, '. New fitness', fitness_train, '. Old fitness:', best_fitness)

                best_fitness = fitness_train
                self.pareto_front[parsim] = tree_meta
    ...
    def pop_parsimony_best_update(self, gene_pool_hash_dict):
        ...
        for tree_id, tree_hash in gene_pool_hash_dict.items():
            ...
            parsim = self.hash_tree_meta[gene_pool_hash_dict[tree_id]]['parsimony']
            fitness_train = self.hash_tree_meta[gene_pool_hash_dict[tree_id]]['fitness_train']

            # 3. is the tree better than the current best in this parsimony level?
            if parsim in self.parsimony_best_hash:
                cmp_fitness = self.hash_tree_meta[self.parsimony_best_hash[parsim]]['fitness_train']
                if (self.fitness_type == 'max' and fitness_train > cmp_fitness) or \
                        (self.fitness_type == 'min' and fitness_train < cmp_fitness):

                    # update everything to the new tree
                    self.parsimony_best_hash[parsim] = gene_pool_hash_dict[tree_id]
    ...
    def tree_store_meta_get_hash(self, tree, store_in_tree=True):
        ...
        if tree_ident in self.hash_tree_meta:
            tree_meta = self.hash_tree_meta[tree_ident]
        ...
        # 2.2 New tree, but still Skip fitness eval for complex trees
            ...
            self.hash_tree_meta[tree_ident] = tree_meta
    ...
    #
    # def pop_tree_store_fitness_parsimony_analysis(self, population, gene_pool_hash_dict):
    #     """
