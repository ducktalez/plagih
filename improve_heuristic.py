...
def get_fitness_samples(agent, sample_size=1000, sample_type="run_experiments"):
    ...
    # To get samples completely arbitrary
    if sample_type == "run_experiments":
        for i in range(sample_size):
            state_action_samples.append(agent.get_action(state, model))
        return
    elif sample_type == "arbitrary_samples":
        # raise print('The option {} in get_fitness_samples() is not yet defined. Using "run_experiments" instead'.format(sample_type))
        print('The option {} in get_fitness_samples() is not yet defined. Using "run_experiments" instead'.format(
            sample_type))
        state_action_samples = get_fitness_samples(agent, sample_size, sample_type="run_experiments")
        return state_action_samples

    # If some crap that does not exist is selected.
    else:
        raise print('The option {} in get_fitness_samples() is not yet defined'.format(sample_type))
