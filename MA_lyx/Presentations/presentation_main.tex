...
\begin{frame}{Example: Layerwise Relevance Propagation (LRP)}
 ...
 \begin{figure}
  ...
  \caption{The LRP marks the decisive pixels in image classification tasks.}
 \end{figure}
  \column{0.40\textwidth}
 \begin{itemize}
  \item Relevance feedback in a NN
  \item Marks the decisive pixels
  \item Can provide local interpretability
 \end{itemize}
 \end{columns}
\end{frame}
...
\bigskip
Manifestations of ``interpretability'' (Doshi-Velez and Kim 17)
 \begin{itemize}
...
\begin{frame}{Genetic Programming}
 \begin{columns}
  \column{0.60\textwidth}
   \begin{figure}
    \includegraphics[height=0.6\textheight]{../img/inkscape/GP-mutations.pdf}
	\caption{Branches between two selected trees of a population are recombined (crossover).}
   \end{figure}
  \column{0.4\textwidth}
   Evolutionary approach (population based)
   \begin{itemize}
	\item Selection
    \item Mutation
    \item Crossover
...
\begin{frame}{Mountain Car ``Black Box'' policy visualisation}
...
\begin{frame}{Industrial Benchmark}
 A benchmark for an industrial control system
 \begin{table}[tph]
